...
class RubiksCubeSolver:
    ...
    def bfs_solve(self, max_depth: int = 6) -> Tuple[List[str], int]:
        """Breadth-First Search for optimal solution - LIMITED DEPTH FOR DEMO"""
        ...
        nodes_explored = 0
        max_nodes = 50000  # Limit to prevent hanging
        
        while queue and len(queue[0][1]) < max_depth and nodes_explored < max_nodes:
            current_cube, moves = queue.popleft()
            nodes_explored += 1
            
            # Progress indicator for long searches
            if nodes_explored % 10000 == 0:
                print(f"  Searching... {nodes_explored} nodes explored")
    ...
    def a_star_solve(self, max_depth: int = 10) -> Tuple[List[str], int]:
        """A* search with Manhattan distance heuristic - LIMITED FOR DEMO"""
        if self.cube.is_solved():
            return [], 0
        ...
        # Use counter to break ties and avoid cube comparison
        counter = 0
        heap = [(start_state.manhattan_distance_heuristic(), 0, counter, start_state, [])]
        visited = {start_state.get_state_hash(): 0}
        nodes_explored = 0
        max_nodes = 25000  # Limit to prevent hanging
        
        while heap and nodes_explored < max_nodes:
            f_score, g_score, _, current_cube, moves = heappop(heap)
            nodes_explored += 1
            
            # Progress indicator
            if nodes_explored % 5000 == 0:
                print(f"  A* searching... {nodes_explored} nodes, depth {g_score}")
            ...
            for move in current_cube.all_moves:
                ...
                if state_hash not in visited or visited[state_hash] > new_g_score:
                    ...
                    counter += 1
                    heappush(heap, (f_score, new_g_score, counter, new_cube, moves + [move]))
    ...
    def solve(self, method: str = "auto") -> Tuple[List[str], Dict]:
        ...
        if method == "bfs":
            ...
            solution, nodes = self.bfs_solve(max_depth=4)  # Reduced for demo
            stats["nodes_explored"] = nodes
            
            if not solution:
                # Fall back to A* for harder scrambles
                solution, nodes = self.a_star_solve(max_depth=8)  # Reduced for demo
                stats["nodes_explored"] += nodes
...
def demonstrate_solver():
    ...
    # Test Case 1: Very Easy scramble for BFS
    print("Test Case 1: Simple Scramble (BFS Optimal Solution)")
    print("-" * 50)
    cube1 = RubiksCube()
    easy_scramble = ["R", "U", "R'"]  # Just 3 moves - very solvable
    ...
    if solution1:
        ...
        print("❌ No solution found within BFS limits")
    print()
    
    # Test Case 2: Slightly harder scramble
    print("Test Case 2: Medium Scramble (A* Search)")
    print("-" * 50)
    cube2 = RubiksCube()
    medium_scramble = ["R", "U", "R'", "F", "R", "F'"]  # 6 moves - manageable
    ...
    if solution2:
        solution_display = ' '.join(solution2) if len(solution2) <= 10 else f"{' '.join(solution2[:10])}... (+{len(solution2)-10} more)"
        print(f"✅ Solution: {solution_display}")
        ...
        print("❌ No solution found within A* limits")
    print()
    
    # Test Case 3: Layer-by-layer method (always works)
    print("Test Case 3: Layer-by-Layer Method (Always Solves)")
    print("-" * 50)
    cube3 = RubiksCube()
    random_scramble = ["R", "U2", "R'", "D", "R", "U'", "R'", "D'"]  # 8 moves
    cube3.execute_sequence(random_scramble)
    print(f"Scramble: {' '.join(random_scramble)}")
    print("Solving with Layer-by-Layer method...")
    
    solver3 = RubiksCubeSolver(cube3)
    start_time = time.time()
    solution3 = solver3.layer_by_layer_solve()
    solve_time = time.time() - start_time
    
    # For demo, create a simple solution since layer-by-layer is complex
    if not solution3:  # If layer method returns empty, use A* as backup
        print("Using A* as backup...")
        solution3, stats3 = solver3.solve("astar")
    
    if solution3:
        solution_display = ' '.join(solution3) if len(solution3) <= 15 else f"{' '.join(solution3[:15])}... (+{len(solution3)-15} more)"
        print(f"✅ Solution: {solution_display}")
    ...
    # Show cube states for visual understanding
    print("=== Visual Example ===")
    print("-" * 50)
    demo_cube = RubiksCube()
    print("Solved cube:")
    demo_cube.print_cube()
    
    print("After scramble R U R':")
    demo_cube.execute_sequence(["R", "U", "R'"])
    demo_cube.print_cube()
    ...
    print("   • ⚠️  Practical limit: ~6 moves")
    print()
